...
export const runFlow = async (message: string): Promise<string> => {
    const api_url = `https://cors-proxy.fringe.zone/https://api.langflow.astra.datastax.com/lf/${LANGFLOW_ID}/api/v1/run/${ENDPOINT}`;
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`HTTP error! saatatus: ${response.status}`);
        }
